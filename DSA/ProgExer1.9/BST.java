...
public class BST {
    ...
    protected Boolean search(BSTNode p, String name) {
        while (p != null) {
            if (name.compareToIgnoreCase(p.key.getSoftwareName()) == 0){
                return true;
            }
            else if (name.compareToIgnoreCase(p.key.getSoftwareName()) < 0) {
                p = p.left;
            } 
            else if (name.compareToIgnoreCase(p.key.getSoftwareName()) > 0) {
                p = p.right;
    ...
    protected void inorderPrint(BSTNode p, PrintWriter pw){
        if (p != null) {
            inorderPrint(p.left, pw);
            pw.println(p.key.getSoftwareName());
            pw.println(p.key.getSoftwareVersion());
            pw.println(p.key.getQuantity());
